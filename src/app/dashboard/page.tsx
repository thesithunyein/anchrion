...
import { Approval, WalletStats, SUPPORTED_CHAINS, NETWORK_NAMES } from '@/types/approval';
import { calculateRiskScore, getRiskLabel } from '@/lib/risk/scorer';
import { fetchApprovals } from '@/lib/graph/client';

const SUPPORTED_CHAIN_IDS = [11155111, 1, 8453, 42161, 10];

function isSupportedChain(id: number): boolean {
  return SUPPORTED_CHAIN_IDS.includes(id);
}

function getChainBadge(chainId: number) {
  if (chainId === 11155111) return { label: 'Testnet', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)' };
  return { label: 'Mainnet', color: '#22c55e', bg: 'rgba(34,197,94,0.12)' };
}
...
export default function Dashboard() {
  ...
  const handleConnect = useCallback((walletId: string) => {
    ...
    try {
      ...
      if (connector) {
        ...
        setConnectError(`${walletId.charAt(0).toUpperCase() + walletId.slice(1)} is not installed. Visit ${installUrl}`);
      }
    } catch (err: any) {
  ...
  const handleSwitchChain = useCallback((targetChainId: number) => {
    if (switchChain) {
      switchChain({ chainId: targetChainId as 11155111 | 1 | 8453 | 42161 | 10 });
    }
  }, [switchChain]);

  useEffect(() => {
    if (isConnected && address && chain && isSupportedChain(chain.id)) {
      loadApprovals();
    }
  ...
  const chainBadge = chain ? getChainBadge(chain.id) : null;
  const unsupportedChain = isConnected && chain && !isSupportedChain(chain.id);
      ...
      <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 24, padding: 32, textAlign: 'center' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 400, lineHeight: 1.6 }}>
            Scan your token approvals and check your wallet's security risk score.
          </p>
        </div>
        {connectError && (
          <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#fca5a5', fontSize: 13, maxWidth: 380, width: '100%' }}>
            {connectError}
          </div>
        )}
        ...
        <p style={{ fontSize: 12, color: 'var(--text-tertiary)', maxWidth: 320, lineHeight: 1.5, marginTop: 4 }}>
          No wallet? Install <a href="https://metamask.io" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--blue-light)' }}>MetaMask</a>, <a href="https://www.coinbase.com/wallet" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--blue-light)' }}>Coinbase Wallet</a>, or <a href="https://phantom.app" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--blue-light)' }}>Phantom</a>.
        </p>
  ...
  if (unsupportedChain) return (
    <div style={{ minHeight: '100vh', position: 'relative' }}>
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, overflow: 'hidden', background: 'linear-gradient(135deg, #0a1628 0%, #0d1f3c 25%, #0f2847 50%, #0a1628 75%, #050a14 100%)' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 800px 600px at 25% 35%, rgba(26,115,232,0.12) 0%, transparent 70%)' }} />
      </div>
      <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, padding: 32, textAlign: 'center' }}>
        ...
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(22px,3vw,28px)', fontWeight: 400, marginBottom: 10 }}>Unsupported Network</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, maxWidth: 380, lineHeight: 1.6 }}>
            You're connected to <strong style={{ color: 'var(--text)' }}>{chain?.name}</strong> (chain {chain?.id}).
            Please switch to a supported network.
          </p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', maxWidth: 300 }}>
          <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--text-tertiary)', marginBottom: 4 }}>Switch to:</p>
          {SUPPORTED_CHAIN_IDS.map(id => {
            const c = SUPPORTED_CHAINS[id as keyof typeof SUPPORTED_CHAINS];
            if (!c) return null;
            const badge = getChainBadge(id);
            return (
              <button key={id} onClick={() => handleSwitchChain(id)}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'var(--text)', fontSize: 14, fontWeight: 500, cursor: 'pointer', transition: 'all .2s' }}>
                <span style={{ padding: '2px 8px', borderRadius: 12, fontSize: 10, fontWeight: 600, background: badge.bg, color: badge.color }}>{badge.label}</span>
                {c.name}
              </button>
            );
          })}
        </div>
        <button onClick={() => disconnect()} style={{ marginTop: 8, fontSize: 13, color: 'var(--text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <header style={{ position: 'sticky', top: 0, zIndex: 50, height: 56, display: 'flex', alignItems: 'center', padding: '0 clamp(20px,4vw,48px)', justifyContent: 'space-between', backdropFilter: 'blur(40px) saturate(1.4)', WebkitBackdropFilter: 'blur(40px) saturate(1.4)', background: 'rgba(8,9,13,0.6)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {/* Network badge — clickable to switch */}
            {chain && (
              <button onClick={() => handleSwitchChain(chain.id === 11155111 ? 1 : 11155111)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, background: chainBadge!.bg, color: chainBadge!.color, border: `1px solid ${chainBadge!.color}30`, cursor: 'pointer', transition: 'opacity .2s' }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: chainBadge!.color }} />
                {chainBadge!.label}: {chain.name}
                <span style={{ fontSize: 10, opacity: 0.7 }}>switch</span>
              </button>
            )}
        ...
        <main style={{ maxWidth: 1120, margin: '0 auto', padding: 'clamp(24px,4vh,40px) clamp(20px,4vw,48px)' }}>
          ...
          {chain && chain.id === 11155111 && (
            <div style={{ marginBottom: 20, padding: '12px 16px', borderRadius: 10, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', fontSize: 13, color: '#fbbf24', lineHeight: 1.5 }}>
              <strong>Sepolia Testnet</strong> — Get free test ETH from <a href="https://cloud.google.com/application/web3/faucet/ethereum/sepolia" target="_blank" rel="noopener noreferrer" style={{ color: '#fbbf24', textDecoration: 'underline' }}>Google Cloud Faucet</a>, then swap tokens on <a href="https://app.uniswap.org" target="_blank" rel="noopener noreferrer" style={{ color: '#fbbf24', textDecoration: 'underline' }}>Uniswap</a> to create approvals.
            </div>
          )}
          ...
          <div className="dash-enter-delay-3">
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '64px 0' }}>
                ...
                <button onClick={loadApprovals} style={{ padding: '10px 24px', borderRadius: 8, fontSize: 14, fontWeight: 600, color: 'white', background: 'var(--blue)', border: 'none', cursor: 'pointer' }}>Try Again</button>
              </div>
            ) : filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '64px 20px', color: 'var(--text-secondary)' }}>
                {approvals.length === 0 ? (
                  <div>
                    <p style={{ fontSize: 16, marginBottom: 12, color: 'var(--text)' }}>No approvals found</p>
                    <p style={{ fontSize: 14, maxWidth: 400, margin: '0 auto', lineHeight: 1.6 }}>
                      {chain?.id === 11155111 ? (
                        <>This wallet has no token approvals on Sepolia. Try swapping tokens on <a href="https://app.uniswap.org" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--blue-light)' }}>Uniswap</a> first.</>
                      ) : (
                        <>This wallet has no token approvals on {chain?.name}. Approvals are created when you swap, stake, or interact with DeFi protocols.</>
                      )}
                    </p>
                    <button onClick={() => handleSwitchChain(11155111)} style={{ marginTop: 16, padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 500, color: 'var(--blue-light)', background: 'rgba(26,115,232,0.1)', border: '1px solid rgba(26,115,232,0.2)', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
            <div>
              ...
              {a.riskFactors.length === 0 ? (
                <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>No risk factors detected</p>
              ) : (
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 13, color: 'var(--text-secondary)' }}>
                ...
                <p><span style={{ color: 'var(--text-tertiary)' }}>Chain: </span>{SUPPORTED_CHAINS[a.chainId as keyof typeof SUPPORTED_CHAINS]?.name || a.chainId}</p>
                <p><span style={{ color: 'var(--text-tertiary)' }}>Token: </span>{a.tokenAddress.slice(0, 18)}...</p>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
            <button style={{ padding: '9px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, color: 'white', background: 'var(--risk-critical)', transition: 'opacity .2s', border: 'none', cursor: 'pointer' }}>
              Revoke Approval
            </button>
            <a href={`${SUPPORTED_CHAINS[a.chainId as keyof typeof SUPPORTED_CHAINS]?.blockExplorer || 'https://etherscan.io'}/token/${a.tokenAddress}?a=${a.spenderAddress}`} target="_blank" rel="noopener noreferrer"
              style={{ padding: '9px 18px', borderRadius: 8, fontSize: 13, color: 'var(--text-secondary)', border: '1px solid rgba(255,255,255,0.08)', background: 'transparent', transition: 'all .2s', textDecoration: 'none' }}>
              View on Explorer
            </a>
          </div>
        </div>
